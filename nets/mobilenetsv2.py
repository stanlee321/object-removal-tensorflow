...
def LiteConv(x, i, filter_num):
    x = Conv2D(filter_num//2, (1, 1), padding='same',
               use_bias=False, name=str(i)+'_pwconv1')(x)
    x = BatchNormalization(momentum=0.99, name=str(i)+'_pwconv1_bn')(x)
    x = Activation('relu', name=str(i)+'_pwconv1_act')(x)
    x = DepthwiseConv2D(kernel_size=3, strides=2, activation=None,
                        use_bias=False, padding='same', name=str(i)+'_dwconv2')(x)
    x = BatchNormalization(momentum=0.99, name=str(i) + '_sepconv2_bn')(x)
    x = Activation('relu', name=str(i) + '_sepconv2_act')(x)
    net = Conv2D(filter_num, (1, 1), padding='same',
                 use_bias=False, name=str(i) + '_pwconv3')(x)
...
def Conv(x, filter_num):
    net = Conv2D(filter_num, kernel_size=1, strides=(
        2, 2), use_bias=False, name='Conv_1')(x)
...
def _isb4conv13(inputs, expansion, stride, alpha, filters, block_id):
    ...
    pointwise_filters = _make_divisible(pointwise_conv_filters, 8)
    # Expand
    conv = Conv2D(expansion * in_channels, kernel_size=1, padding='same',
                  use_bias=False, activation=None, name='mobl%d_conv_expand' % block_id)(inputs)
    x = BatchNormalization(epsilon=1e-3, momentum=0.999,
                           name='bn%d_conv_bn_expand' % block_id)(conv)
    x = Activation(relu6, name='conv_%d_relu' % block_id)(x)
    # Depthwise
    x = DepthwiseConv2D(kernel_size=3, strides=stride, activation=None,
                        use_bias=False, padding='same', name='mobl%d_conv_depthwise' % block_id)(x)
...
def prediction(x, i, num_priors, min_s, max_s, aspect, num_classes, img_size):
    a = Conv2D(num_priors*4, (3, 3), padding='same',
               name=str(i)+'_mbox_loc')(x)
    mbox_loc_flat = Flatten(name=str(i)+'_mbox_loc_flat')(a)
    b = Conv2D(num_priors*num_classes, (3, 3),
               padding='same', name=str(i)+'_mbox_conf')(x)
    mbox_conf_flat = Flatten(name=str(i)+'_mbox_conf_flat')(b)
    mbox_priorbox = PriorBox(img_size, min_size=min_s, max_size=max_s, aspect_ratios=aspect, variances=[
                             0.1, 0.1, 0.2, 0.2], name=str(i)+'_mbox_priorbox')(x)
    return mbox_loc_flat, mbox_conf_flat, mbox_priorbox


def SSD(input_shape, num_classes, alpha):
    """SSD300 architecture.
    # Arguments
    ...
    # Plot backbone mobnetsv2
    plot_model(mobilenetv2, to_file='model_plot_mobnetv2.png',
               show_shapes=True, show_layer_names=True)
    
    # Get Features for black_16 layer
    FeatureExtractor = Model(
        inputs=mobilenetv2.input, outputs=mobilenetv2.get_layer('block_16_project_BN').output)

    x = FeatureExtractor(Input0)
    x, pwconv3 = _isb4conv13(x, filters=160, alpha=alpha,
                             stride=1, expansion=6, block_id=13)
    # x=get_3rd_layer_output([x,1])[0]
    x = _inverted_res_block(x, filters=160, alpha=alpha,
                            stride=1, expansion=6, block_id=14)
    x = _inverted_res_block(x, filters=160, alpha=alpha,
                            stride=1, expansion=6, block_id=15)
    x = _inverted_res_block(x, filters=320, alpha=alpha,
                            stride=1, expansion=6, block_id=16)
...
if __name__ == '__main__':
    model = SSD((224, 224), 10, 1.0)
    #model.load_weights('./MobileNetV2SSD300Liteweights.14-0.77.hdf5', by_name=True)
    model.summary()
