...
The general idea is to use the (object segmentation + inpainting process) for accomplish the Dynamic Object Removal task.

For this we use three Networks, [DeepLabv3+(mobilenetsV2 backbone)](https://github.com/stanlee321/object-removal-tensorflow/tree/master/deeplab3) + [customlayer](https://github.com/stanlee321/object-removal-tensorflow/blob/master/model.py) + [glcic](https://github.com/stanlee321/object-removal-tensorflow/tree/master/inpainting).

The first network computes the `mask segmentations` the second insert the  `white holes` in the image  and the thrid `fill the white holes` using a [GAN](https://github.com/stanlee321/object-removal-tensorflow/tree/master/inpainting) .
